...
import os
# Set your Groq API key here (replace <YOUR_GROQ_API_KEY> with your actual key)
...
def process_row(idx, row, groq_client, prompt_template, dict_str, top_words_col, top_bigrams_col, top_trigrams_col):
    """
    Processes a single row to extract subtopics.
    Returns a tuple (index, subtopics_list).
    """
    ...
    try:
        ...
        try:
            subtopics_list = ast.literal_eval(response_text)
...
def generate_expertise(df, groq_client=None,
    ...
    """
    Processes the input DataFrame in parallel (using a thread pool) to extract subtopics.
    Returns the updated DataFrame with a new "Subtopics" column.
    
    Note: Adjust max_workers as needed. Be mindful of API rate limits.
    """
    ...
    dict_str = ""
    for parent, sub_str in subtopics_dict.items():
        dict_str += parent + ":\n" + sub_str.strip() + "\n\n"
    
    prompt_template = """
You are an expert astrophysicist. Based solely on the provided ordered lists of n-grams extracted from a researcher's ADS abstracts, please determine 1 to 3 subtopics that best represent the researcher's work, along with their corresponding parent topics. Use the following dictionary of parent topics and their allowed subtopics:

{dictionary}

For each selected subtopic, return a string in the following format:
"parent topic:subtopic"

For example: "stars:stellar evolution", "cosmology:dark energy".

Return your answer as a Python list (e.g. ["stars:stellar evolution", "cosmology:dark energy"]). Do not include any additional commentary or text.

Researcher's n-grams:
...
"""
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_idx = {executor.submit(process_row, idx, row, groq_client, prompt_template, dict_str,
                            top_words_col, top_bigrams_col, top_trigrams_col): idx for idx, row in df.iterrows()}
    ...
    # Create a list of results in the order of the DataFrame's index.
